layui.use(['form', 'table'], function () {
    ...
    table.render({
        ...
        , cols: [[ //表头
            ...
            , {field: 'parameter', title: '响应参数', align: 'center'}
    ...
    table.on('sort(apiResponseFilter)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
    ...
    //监听工具条
    table.on('tool(apiResponseFilter)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if (layEvent === 'del') { //删除
            ...
            layer.open({
                ...
                , content: '/ApiResponse/getEdit?id=' + data.id //这里content是一个普通的String
    ...
    $('#add').on('click', function () {
        layer.open({
            ...
            , shadeClose: true //开启遮罩关闭
            , title: '信息增加'
            , content: "/ApiResponse/goAdd" //这里content是一个普通的String
            , area: [(w - 400) + 'px', (h - 150) + 'px']
            , closeBtn: 0 //关闭按钮
        });
    });
